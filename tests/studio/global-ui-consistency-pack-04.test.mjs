...
test("one canonical graphite action palette drives shared controls",()=>{assert.match(globals,/--studio-action-primary:\s*#444b51/);assert.match(globals,/--studio-action-primary-hover:\s*#525a61/i);assert.match(globals,/--studio-action-primary-pressed:\s*#393f44/i);for(const selector of["studio-button--primary","studio-tab-active","studio-tab--icon-navigation.studio-tab-active","studio-tab--workspace-navigation.studio-tab-active","studio-segmented button.is-active"])assert.match(globals,new RegExp(`${selector.replaceAll(".","\\.")}[^}]*var\\(--studio-action-primary\\)`))});
