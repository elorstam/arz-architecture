...
test("one canonical sidebar-derived action palette drives shared controls",()=>{assert.match(globals,/--studio-sidebar-bg:\s*#111923/);assert.match(globals,/--studio-action-primary:\s*var\(--studio-sidebar-bg\)/);assert.match(globals,/--studio-action-primary-hover:\s*var\(--studio-navy\)/);assert.match(globals,/--studio-action-primary-pressed:\s*color-mix\(in srgb, var\(--studio-sidebar-bg\) 88%, #000\)/);for(const selector of["studio-button--primary","studio-tab-active","studio-tab--icon-navigation.studio-tab-active","studio-tab--workspace-navigation.studio-tab-active","studio-segmented button.is-active"])assert.match(globals,new RegExp(`${selector.replaceAll(".","\\.")}[^}]*var\\(--studio-action-primary\\)`))});
...
test("Client supporting typography has enforced pixel floors",()=>{assert.match(client,/studio-page-description[^}]*font-size:15px!important/);assert.match(client,/client-payment-request__description[^}]*font-size:14px!important/);assert.match(client,/client-payment-request dt[^}]*font-size:13px!important/);assert.match(client,/client-payment-request dd[^}]*font-size:14px!important/);assert.match(client,/studio-empty-v2 p \{ font-size:14px!important/);assert.match(client,/client-notification-meta[^}]*font-size:13px!important/)});
