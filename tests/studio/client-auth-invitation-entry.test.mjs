...
test("client login and invitation routes are isolated from Studio auth",async()=>{
 ...
 assert.match(layout,/serverAppPath\("client","\/client\/login"\)/);assert.doesNotMatch(layout,/studio\/login/);
...
test("invitation token remains hash-only and transient",async()=>{
 ...
 assert.match(owner,/studio_create_client_invitation/);assert.match(owner,/createClientInvitationUrl/);assert.doesNotMatch(owner,/from\("studio_client_invitations"\).*insert/);
