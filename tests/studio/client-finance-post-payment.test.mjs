...
test("payment result does not remount finance and drawer remains initially closed",()=>{assert.doesNotMatch(route,/key=\{`finance-/);assert.match(list,/useState<string\|null>\(null\)/);assert.doesNotMatch(route+notice,/setSelectedPaymentId|selectedFinance|entryId/);assert.match(list,/selectedPayment\?<ClientFinancePaymentDrawer/);});

test("finance detail drawer uses stable local id state without route navigation",()=>{assert.match(list,/entries\.find\(payment=>payment\.id===selectedPaymentId\)/);assert.match(list,/onClick=\{\(\)=>setSelectedPaymentId\(item\.id\)\}/);assert.match(list,/useCallback\(\(\)=>setSelectedPaymentId\(null\),\[\]\)/);assert.match(list,/onClose=\{closeDrawer\}/);assert.doesNotMatch(list,/useRouter|useSearchParams|router\.|location\.|fetch\(/);});
