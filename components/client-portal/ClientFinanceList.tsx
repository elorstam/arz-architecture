...
import {StudioBadge,StudioDrawer,StudioSegmentedControl,StudioSelect} from "@/components/studio/ui/StudioUiPrimitives";
...
export default function ClientFinanceList({projectName,entries,downloadableFileIds,today}:{projectName:string;entries:ClientFinance[];downloadableFileIds:string[];today:string}){
 ...
 const selected=useMemo(()=>entries.find(item=>item.id===selectedPaymentId)??null,[entries,selectedPaymentId]);
 ...
 return <>
  ...
  <StudioDrawer open={selectedPaymentId!==null} title={selected?.title??"Ödeme Detayı"} onClose={closeDrawer}>{selected?<div className="client-finance-detail"><div><StudioBadge variant={statusVariant(selected.status)}>{statusLabels[selected.status]??selected.status}</StudioBadge>{overdue(selected,today)?<StudioBadge variant="danger">Gecikti</StudioBadge>:null}</div>{selected.description?<section><h3>Açıklama</h3><p>{selected.description}</p></section>:null}<dl><div><dt>Proje</dt><dd>{projectName}</dd></div><div><dt>Tür</dt><dd>{typeLabels[selected.entry_type]??selected.entry_type}</dd></div><div><dt>Tutar</dt><dd>{money(selected)}</dd></div><div><dt>Vade</dt><dd>{date(selected.due_date)}</dd></div><div><dt>Kayıt tarihi</dt><dd>{date(selected.created_at)}</dd></div></dl>{selected.document_file_id&&downloadable.has(selected.document_file_id)?<a href={`/client/files/${selected.document_file_id}/download`} className={studioButtonClass("primary","sm")} aria-label={`${selected.title} belgesini güvenli şekilde indir`}>Belgeyi İndir</a>:null}</div>:null}</StudioDrawer>
 </>;
}
