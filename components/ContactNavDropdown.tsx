...
export default function ContactNavDropdown({label,contactHref,paymentHref,active}:Props){
  ...
  return <div ref={rootRef} className="contact-nav-dropdown relative" onMouseEnter={()=>{cancelClose();setOpen(true);}} onMouseLeave={closeSoon} onFocusCapture={()=>{cancelClose();setOpen(true);}} onBlur={onBlur} onKeyDown={onRootKeyDown}>
    <button ref={triggerRef} type="button" aria-haspopup="menu" aria-expanded={open} aria-controls="contact-navigation-menu" onClick={()=>setOpen(value=>!value)} onKeyDown={onTriggerKeyDown} className={`group relative flex items-center gap-1.5 py-3 text-[12px] font-normal uppercase tracking-[0.11em] transition-[color,transform] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] 2xl:text-[14px] 2xl:tracking-[0.13em] ${active?"text-white/90":"text-white/62 hover:-translate-y-px hover:text-white/90"}`}>
    ...
    <div className={`absolute left-1/2 top-full z-[90] w-[210px] -translate-x-1/2 pt-2 transition-[opacity,transform,visibility] duration-200 ${open?"visible translate-y-0 opacity-100":"pointer-events-none invisible -translate-y-1 opacity-0"}`}>
      <div id="contact-navigation-menu" role="menu" aria-label={label} className="contact-nav-dropdown__panel border p-1 shadow-[0_14px_38px_rgba(0,0,0,.18)]">
        <Link ref={firstItemRef} role="menuitem" href={contactHref} onClick={()=>setOpen(false)} className="contact-nav-dropdown__item flex min-h-11 items-center justify-between px-4 text-[11px] uppercase tracking-[.12em]"><span>Teklif Al</span><span aria-hidden>→</span></Link>
        <Link role="menuitem" href={paymentHref} onClick={()=>setOpen(false)} className="contact-nav-dropdown__item flex min-h-11 items-center justify-between px-4 text-[11px] uppercase tracking-[.12em]"><span>Online Ödeme</span><span aria-hidden>→</span></Link>
      </div>
    </div>
  </div>;
}
