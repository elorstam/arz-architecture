...
export default function StudioCrmPage({leads,totalCount,canManage,filters,members,favoriteKeys}:{leads:StudioLead[];totalCount:number;canManage:boolean;filters:LeadQueryFilters;members:StudioLeadMember[];favoriteKeys:Set<string>}){
 ...
 return <section className="mx-auto min-w-0 max-w-[1540px] px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-9">
  ...
  {leads.length?<div className="mt-5 grid min-w-0 gap-4 md:grid-cols-2 xl:grid-cols-3">{leads.map(lead=><StudioLeadCard key={lead.id} lead={lead} isFavorite={favoriteKeys.has(`crm_lead:${lead.id}`)}/>)}</div>:<StudioLeadEmptyState filtered={filtered||totalCount>0} canManage={canManage}/>}
 </section>;
}
