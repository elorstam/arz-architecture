...
export default function StudioCrmFilters({filters,members}:{filters:LeadQueryFilters;members:StudioLeadMember[]}){
 const field="studio-filter-control min-w-0 rounded-lg border border-[#dcd8cf] bg-white px-4 text-[#4f5657] outline-none focus:border-[#a58a56] focus:ring-2 focus:ring-[#a58a56]/15";
 return <form method="get" className="mt-5 grid gap-3 rounded-xl border border-[#dedad1] bg-white p-4 sm:grid-cols-2 lg:grid-cols-6">
  ...
  <label className="flex min-h-11 items-center gap-2 text-sm font-semibold"><input type="checkbox" name="favorites" value="1" defaultChecked={filters.favoritesOnly}/>Sadece Favoriler</label><div className="flex flex-wrap gap-2 lg:col-span-5 lg:justify-end"><Link href="/studio/crm" className={studioButtonClass("outline", "sm")}>Temizle</Link><button className={studioButtonClass("primary", "sm")}>Filtrele</button></div>
 </form>;
}
