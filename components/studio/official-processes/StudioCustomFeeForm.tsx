...
export default function StudioCustomFeeForm({ projectId }: { projectId: string }) {
  ...
  return <form ref={formRef} action={action} className="grid min-w-0 gap-3 rounded-xl border border-[var(--studio-border)] bg-white p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><label className="min-w-0 text-sm font-semibold">Özel Harç Ekle<input required maxLength={160} name="title" placeholder="Harç adı" className="mt-2 h-11 w-full min-w-0 rounded-lg border border-[var(--studio-border)] bg-white px-3 text-[15px]"/></label><button disabled={pending} className={studioButtonClass("primary")}>{pending?"Oluşturuluyor…":"Oluştur"}</button>{state.message?<p role={state.success?"status":"alert"} className="text-sm sm:col-span-2">{state.message}</p>:null}</form>;
}
