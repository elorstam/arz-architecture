      ...
      <div className="px-5 py-1">
        {items.map((item, index) => (
          <article key={`${item.project}-${item.event}`} className="relative flex gap-3 py-4">
            ...
            <span className="relative z-10 grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#1b2731] text-[7px] font-semibold text-[#d6ba7e]">{item.actor}</span>
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-[9px] leading-4 text-[#777b78]">{item.event}</p>
