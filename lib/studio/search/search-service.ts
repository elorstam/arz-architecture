...
export async function searchStudio(query:string):Promise<StudioSearchResponse>{
  ...
  const auth=await getStudioContext();if(!auth?.user||!auth.membership)throw new Error("unauthorized");
  const client=await createStudioServerClient(),context={organizationId:auth.membership.organization_id,supabase:client};
  const[settled,favoriteResult]=await Promise.all([Promise.allSettled(providers.map(provider=>provider.search(clean,context))),client.from("studio_user_favorites").select("entity_type,entity_id").eq("organization_id",context.organizationId).eq("user_id",auth.user.id).is("archived_at",null)]);
  const favorites=new Set((favoriteResult.data??[]).map(row=>`${row.entity_type}:${row.entity_id}`)),grouped=new Map<string,StudioSearchResponse["groups"][number]>();
  const entityType=(provider:string)=>provider==="projects"?"project":provider==="crm"?"crm_lead":provider==="proposals"?"quote":provider==="files"||provider==="renders"?"file":provider==="file_versions"?"file_version":provider==="folders"?"folder":"";
  providers.forEach((provider,index)=>{const value=settled[index],results=value.status==="fulfilled"?value.value:[];for(const result of results){const type=entityType(result.provider);result.isFavorite=Boolean(type&&favorites.has(`${type}:${result.favoriteEntityId??result.id}`));const key=result.category,current=grouped.get(key);if(current)current.results.push(result);else grouped.set(key,{category:key,label:result.categoryLabel||provider.getLabel(),results:[result]});}});
  const groups=[...grouped.values()];for(const group of groups)group.results.sort((a,b)=>Number(b.isFavorite)-Number(a.isFavorite));return{query:clean,groups,total:groups.reduce((sum,group)=>sum+group.results.length,0)};
}
