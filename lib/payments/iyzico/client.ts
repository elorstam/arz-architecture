...
export async function initializeCheckout(config:IyzicoConfig,payload:Record<string,unknown>){const enabledInstallments=Array.isArray(payload.enabledInstallments)?payload.enabledInstallments.filter(value=>Number.isInteger(value)).map(Number):[];console.info("IYZICO_INITIALIZE_INSTALLMENTS",{enabledInstallments});const response=await call(config,INITIALIZE_PATH,payload);if(response.status!=="success"){const code=String(response.errorCode||"PAYMENT_INITIALIZE_FAILED"),details={stage:"iyzico_initialize_provider_failure",...providerFields(response)};logFailure({code,...details});throw new IyzicoError(code,details);}if(!verifyInitializeSignature(config.secretKey,response)){const details={stage:"iyzico_initialize_signature",providerStatus:safeText(response.status,60)};logFailure({code:"PAYMENT_PROVIDER_SIGNATURE_INVALID",...details});throw new IyzicoError("PAYMENT_PROVIDER_SIGNATURE_INVALID",details);}return response;}
