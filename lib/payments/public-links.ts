...
export async function resolvePublicPayment(token:string,markOpened=false):Promise<ResolvedPublicPayment|null>{
 if(!/^[A-Za-z0-9_-]{43}$/.test(token))return null;const{data,error}=await admin().rpc("resolve_public_payment_link",{p_token_hash:sha256(token),p_mark_opened:markOpened});const row=(Array.isArray(data)?data[0]:data)as Record<string,unknown>|undefined;if(error||!row)return null;
 let identity:string;try{identity=decryptCheckoutPaymentIdentity(String(row.buyer_identity_number_encrypted),String(row.buyer_identity_number_last_two));}catch{return null;}
 return{linkId:String(row.link_id),createdBy:String(row.created_by),expiresAt:String(row.expires_at),linkStatus:String(row.link_status)as PublicLinkStatus,payment:{id:String(row.payment_request_id),organization_id:String(row.organization_id),project_id:String(row.project_id),project_name:String(row.project_name),title:String(row.title),amount:String(row.amount),currency:String(row.currency),status:String(row.payment_status)as CheckoutPayment["status"],buyer_id:String(row.link_id),buyer_full_name:String(row.buyer_full_name),buyer_identity_number:identity,buyer_email:String(row.buyer_email),buyer_gsm_number:String(row.buyer_gsm_number),buyer_registration_address:String(row.buyer_registration_address),buyer_city:String(row.buyer_city),buyer_country:String(row.buyer_country),buyer_zip_code:row.buyer_zip_code?String(row.buyer_zip_code):null}};
}
