...
export async function getLocalizedStoreProjects(locale:string){return (await getManagedProjects()).filter(p=>p.published).map(p=>({...locale==='tr'?p.tr:p.en,slug:locale==='tr'?p.slugTr:p.slugEn}));}
