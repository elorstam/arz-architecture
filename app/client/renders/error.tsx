...
export default function ClientRendersError({unstable_retry}:{error:Error&{digest?:string};unstable_retry:()=>void}){return <div className="client-error-surface"><StudioEmptyState icon="warning" title="Renderlar şu anda yüklenemedi." description="Lütfen kısa bir süre sonra yeniden deneyin." action={<button className={studioButtonClass("primary","sm")} onClick={unstable_retry}>Yeniden Dene</button>}/></div>;}
