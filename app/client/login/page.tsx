...
export default async function ClientLoginPage({searchParams}:{searchParams:Promise<{next?:string;error?:string}>}){
 const query=await searchParams;const destination=await resolveAuthenticatedDestination().catch(()=>({kind:"unauthenticated" as const}));if(destination.kind==="client")redirect(await serverAppPath("client",safeClientNext(query.next)));if(destination.kind==="staff")redirect(await serverAppPath("studio","/studio"));
