...
const allowed=new Set<StudioAiOperation>(["crm_ai_meeting_note","proposal_ai_description","decision_ai_summary","stage_ai_description","render_description","finance_summary","payment_reminder","invoice_description","progress_payment","cashflow_summary"]);
export async function generateStudioFormTextAction(operation:StudioAiOperation,context:Record<string,string|string[]|null>,currentText:string,format:StudioAiOutputFormat){
 try{
  ...
  const financeOperations=new Set(["finance_summary","payment_reminder","invoice_description","progress_payment","cashflow_summary"]);const moduleName=operation==="crm_ai_meeting_note"?"crm":operation==="proposal_ai_description"?"proposals":operation==="render_description"?"renders":financeOperations.has(operation)?"finance":"decision_log";
