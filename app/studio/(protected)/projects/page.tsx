...
type SearchParams=Promise<{q?:string;status?:string;stage?:string;archive?:string;tags?:string|string[];tagMode?:string}>;
export default async function ProjectsPage({searchParams}:{searchParams:SearchParams}){
 ...
 const tagIds=(Array.isArray(params.tags)?params.tags:(params.tags??"").split(",")).filter(Boolean);const tagMode=params.tagMode==="all"?"all":"any";const[projects,allProjects,access,matched]=await Promise.all([getStudioProjects({query,status,stage,archive}),getStudioProjects({archive:"all"}),getStudioProjectAccess(),filterEntityIdsByTags("project",tagIds,tagMode)]);const filtered=matched?projects.filter(project=>matched.has(project.id)):projects;
 return <><StudioTagFilterForm selected={tagIds} mode={tagMode}/><StudioProjectsPage projects={filtered} totalCount={allProjects.length} canManage={access.canManage} filters={{query,status,stage,archive}}/></>;
}
