...
import {getStudioProjectAccess,getStudioProjectMembers} from "@/lib/studio/projects/project-repository";

export const dynamic="force-dynamic";
export default async function NewProjectPage(){
 const access=await getStudioProjectAccess();if(!access.canManage)redirect("/studio/projects");
 const members=await getStudioProjectMembers();
 return <section className="mx-auto min-w-0 max-w-5xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-9">
  ...
  <StudioProjectForm action={createStudioProjectAction} members={members} mode="create"/>
 </section>;
}
