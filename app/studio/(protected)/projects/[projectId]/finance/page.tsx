...
export default async function Page({params}:{params:Promise<{projectId:string}>}){
 ...
 const[data,paymentRequests]=await Promise.all([getProjectFinanceWorkspace(projectId),getStudioPaymentRequests(projectId)]);
 return <main className="studio-finance-page mx-auto min-w-0 max-w-[1600px] px-4 py-5 sm:px-6 lg:px-8">
  ...
  <StudioPaymentRequests projectId={projectId} canManage={data.canManage} requests={paymentRequests}/>
