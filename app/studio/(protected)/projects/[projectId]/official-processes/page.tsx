...
export default async function OfficialProcessesPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return <main className="mx-auto min-w-0 max-w-[1540px] px-4 py-6 sm:px-6 lg:px-8"><PageHeader code={project.code} name={project.name} projectId={projectId}/><StudioOfficialProcessSummaryGrid items={items}/><ProcessSection title="Belediye Harçları" description="Tahakkuk, ödeme ve alınan evrak takibi." count={`${fees.length} kayıt`}>{access.canManage?<StudioCustomFeeForm projectId={projectId}/>:null}<div className="grid min-w-0 gap-4 xl:grid-cols-2">{fees.map(item=><StudioOfficialProcessCard key={item.id} item={item} documents={documents} canManage={access.canManage}/>)}</div></ProcessSection><ProcessSection title="Aplikasyon" description="Projenin aplikasyon tahakkuk, ödeme ve nihai evrak süreci." count={application[0]?.status??"Bekleniyor"}><div className="grid min-w-0 gap-4">{application.map(item=><StudioOfficialProcessCard key={item.id} item={item} documents={documents} canManage={access.canManage}/>)}</div></ProcessSection><ProcessSection title="Temiz Aplikasyon" description="Normal aplikasyon sürecinden bağımsız temiz aplikasyon takibi." count={cleanApplication[0]?.status??"Bekleniyor"}><div className="grid min-w-0 gap-4">{cleanApplication.map(item=><StudioOfficialProcessCard key={item.id} item={item} documents={documents} canManage={access.canManage}/>)}</div></ProcessSection></main>;
}
