...
export default async function OfficialProcessesPage({params,searchParams}:{params:Promise<{projectId:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <main className="mx-auto min-w-0 max-w-[1540px] px-4 py-5 sm:px-6 lg:px-8">
  <StudioPageHeader eyebrow={project.code} title="Harç ve Evraklar" description={`${project.name} için belediye harçlarını, resmî evrakları ve yaklaşan işlemleri yönetin.`} icon="receipt" actions={access.canManage&&!archiveView?<details className="relative"><summary className="studio-button studio-button--primary cursor-pointer list-none">Özel Harç Ekle</summary><div className="absolute right-0 top-12 z-30 w-[min(92vw,440px)] rounded-2xl border border-[var(--studio-border)] bg-white p-3 shadow-xl"><StudioCustomFeeForm projectId={projectId}/></div></details>:null}/>
