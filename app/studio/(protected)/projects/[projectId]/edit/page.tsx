...
import {getStudioProjectAccess,getStudioProjectById,getStudioProjectMembers,listStudioProjectTypes} from "@/lib/studio/projects/project-repository";
...
export default async function EditProjectPage({params}:{params:Promise<{projectId:string}>}){
 ...
 const[project,members,projectTypes]=await Promise.all([getStudioProjectById(projectId),getStudioProjectMembers(),listStudioProjectTypes(true)]);if(!project)notFound();
 ...
 return <section className="mx-auto min-w-0 max-w-5xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-9">
  ...
  <StudioProjectForm action={action} initialValues={projectToFormValues(project)} members={members} projectTypes={projectTypes} mode="edit" projectId={project.id}/>
 </section>;
}
