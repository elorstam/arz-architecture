...
type SearchParams=Promise<{q?:string;stage?:string;status?:string;serviceType?:string;assignedUserId?:string;archive?:string;tags?:string|string[];tagMode?:string;favorites?:string}>;
export default async function CrmPage({searchParams}:{searchParams:SearchParams}){
 ...
 const filters={query,stage,status,serviceType,assignedUserId,archive,favoritesOnly:params.favorites==="1"};
 const tagIds=(Array.isArray(params.tags)?params.tags:(params.tags??"").split(",")).filter(Boolean);const tagMode=params.tagMode==="all"?"all":"any";const[leads,allLeads,access,matched,favoriteKeys]=await Promise.all([getStudioLeads(filters),getStudioLeads({archive:"all"}),getStudioLeadAccess(),filterEntityIdsByTags("crm_lead",tagIds,tagMode),listUserFavoriteKeys()]);let filtered=matched?leads.filter(lead=>matched.has(lead.id)):leads;if(filters.favoritesOnly)filtered=filtered.filter(lead=>favoriteKeys.has(`crm_lead:${lead.id}`));
 return <><StudioTagFilterForm selected={tagIds} mode={tagMode}/><StudioCrmPage leads={filtered} totalCount={allLeads.length} canManage={access.canManage} filters={filters} members={members} favoriteKeys={favoriteKeys}/></>;
}
