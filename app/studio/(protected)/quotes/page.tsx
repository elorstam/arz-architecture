...
type SearchParams=Promise<{q?:string;status?:string;currency?:string;leadId?:string;archive?:string;createdFrom?:string;createdTo?:string;validFrom?:string;validTo?:string;tags?:string|string[];tagMode?:string;favorites?:string}>;
...
export default async function QuotesPage({searchParams}:{searchParams:SearchParams}){const params=await searchParams;const leads=await getStudioQuoteLeads(true);
 const filters={query:params.q?.trim().slice(0,120)||"",status:QUOTE_STATUSES.find(value=>value===params.status),currency:QUOTE_CURRENCIES.find(value=>value===params.currency),
  ...
  createdFrom:date(params.createdFrom),createdTo:date(params.createdTo),validFrom:date(params.validFrom),validTo:date(params.validTo),favoritesOnly:params.favorites==="1"};
 const tagIds=(Array.isArray(params.tags)?params.tags:(params.tags??"").split(",")).filter(Boolean);const tagMode=params.tagMode==="all"?"all":"any";const[quotes,all,access,matched,favoriteKeys]=await Promise.all([getStudioQuotes(filters),getStudioQuotes({archive:"all"}),getStudioQuoteAccess(),filterEntityIdsByTags("quote",tagIds,tagMode),listUserFavoriteKeys()]);let filtered=matched?quotes.filter(quote=>matched.has(quote.id)):quotes;if(filters.favoritesOnly)filtered=filtered.filter(quote=>favoriteKeys.has(`quote:${quote.id}`));
 return <><StudioTagFilterForm selected={tagIds} mode={tagMode}/><StudioQuotesPage quotes={filtered} total={all.length} canManage={access.canManage} filters={filters} leads={leads} favoriteKeys={favoriteKeys}/></>;
}
