...
export async function generateMetadata({params}:Props):Promise<Metadata>{
  ...
  if(!managed)return {title:locale==="en"?"Project Not Found":"Proje Bulunamadı"};
  const project=locale==='en'?managed.en:managed.tr; const segment=locale==="tr"?"projeler":"projects"; const canonicalSlug=locale==='en'?managed.slugEn:managed.slugTr;
...
export default async function Page({params}:Props){const {locale,slug}=await params;const managed=await getManagedBySlug(slug);if(!managed||!managed.published)notFound();const canonicalSlug=locale==='en'?managed.slugEn:managed.slugTr;if(slug!==canonicalSlug)redirect(`/${locale}/${locale==='tr'?'projeler':'projects'}/${canonicalSlug}`);return <ProjectDetail project={locale==='en'?managed.en:managed.tr}/>;}
