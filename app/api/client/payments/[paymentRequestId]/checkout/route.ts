...
import {initializeCheckout,IyzicoError,type IyzicoErrorDetails} from "@/lib/payments/iyzico/client";
...
export async function POST(request:Request,{params}:{params:Promise<{paymentRequestId:string}>}){
 ...
 let lookup;try{lookup=await authenticatedPayment(parsed.data);}catch(error){const code=error instanceof PaymentProfileLoadError?error.code:"PAYMENT_LOOKUP_UNAVAILABLE";checkoutError(error instanceof PaymentProfileLoadError?"checkout_load_profile":"checkout_load_request",{paymentRequestId:parsed.data,normalizedCode:code,...safeException(error)});return json({error:"Ödeme talebi doğrulanamıyor.",code:"PAYMENT_PROFILE_UNAVAILABLE"},503);}if(lookup.kind==="unauthorized")return json({error:"Yetkisiz.",code:"UNAUTHORIZED"},401);if(lookup.kind==="not_found")return json({error:"Ödeme talebi bulunamadı.",code:"PAYMENT_NOT_FOUND"},404);
 const payment=lookup.payment;if(payment.status==="paid")return json({error:"Bu ödeme zaten tamamlandı.",code:"PAYMENT_ALREADY_PAID"},409);if(payment.status==="refunded")return json({error:"Bu ödeme iade edildi.",code:"PAYMENT_REFUNDED"},409);if(payment.status==="cancelled")return json({error:"Bu ödeme talebi iptal edildi.",code:"PAYMENT_CANCELLED"},409);
 ...
 }catch(error){const code=error instanceof IyzicoError?error.code:"PAYMENT_INITIALIZE_FAILED",details:IyzicoErrorDetails=error instanceof IyzicoError?error.details:{stage:"checkout_unexpected_error",...safeException(error)};checkoutError(details.stage,{paymentRequestId:parsed.data,attemptId:attempt.id,conversationId:attempt.conversation_id,normalizedCode:code,httpStatus:details.httpStatus,providerStatus:details.providerStatus,iyzicoErrorCode:details.providerErrorCode,iyzicoErrorMessage:details.providerErrorMessage,exceptionName:details.exceptionName,exceptionMessage:details.exceptionMessage,configuration:iyzicoConfigStatus()});try{await markAttemptFailed(attempt.id,code);}catch(markError){checkoutError("checkout_unexpected_error",{paymentRequestId:parsed.data,attemptId:attempt.id,conversationId:attempt.conversation_id,normalizedCode:"PAYMENT_ATTEMPT_FAILURE_PERSIST_FAILED",...safeException(markError)});}return json({error:"PAYMENT_CHECKOUT_FAILED"},502);}
}
