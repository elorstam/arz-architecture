...
export async function POST(request:Request){
 try{
  ...
  console.error("Client login failed",{errorType:error instanceof Error?error.name:"unknown"});
  return NextResponse.json({error:"Client giriş servisi şu anda kullanılamıyor."},{status:503});
 }
}
