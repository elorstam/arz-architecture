...
import{verifyIyzicoWebhookV3}from"@/lib/payments/iyzico/signatures";
...
export async function POST(request:Request){
 ...
 if(attempt.status==="succeeded"||attempt.status==="refunded")return response(204);
 try{
  const result=await retrieveCheckout(config,attempt.conversation_id,input.data.token);
  if(result.token!==input.data.token)throw new Error("PAYMENT_VERIFICATION_MISMATCH");const metadata=verifyCheckoutResult(result,attempt,String(input.data.iyziPaymentId));
  await finalizeAttempt(attempt.id,String(result.paymentId),String(attempt.amount),attempt.currency,config.environment,metadata);return response(204);
