...
export async function POST(request:Request,{params}:{params:Promise<{token:string}>}){
 ...
 const names=String(payment.buyer_full_name).trim().split(/\s+/),surname=names.length>1?names.pop()!:names[0],name=names.join(" ")||surname,price=Number(attempt.amount),callbackUrl=new URL(`/api/payments/public/${encodeURIComponent(token)}/callback`,appBaseUrl("public"));
