...
export async function POST(request:Request){
 try{
  ...
  await supabase.auth.signOut();return NextResponse.json({success:true,destination:appDestination("studio","/studio/login",request.headers.get("x-forwarded-host")||request.headers.get("host"))});
